...
public static class Bitboard
{
    ...
    // This is for precomputation and I hardcode the values generated by this algorithm
    private static ulong[] GetMaskBetweenSquares(int startIndex)
    {
        ulong[] masks = new ulong[64];
        masks[startIndex] = 1ul << startIndex;
        foreach (int dir in Direction.directions)
        {
            ulong mask = 1ul << startIndex;
            ulong position = 1ul << startIndex;
            int cur = startIndex;

            while (!CheckAtEdgeOfBoard(dir, position))
            {
                cur += dir;
                position = ShiftLeft(position, dir);
                mask |= position;
                masks[cur] = mask;
            }
        }
